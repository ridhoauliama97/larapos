        ...
        <div className={`flex flex-col gap-2 ${className}`}>
            ...
            <div ref={containerRef} className="relative w-full min-w-0">
                <button
                    ...
                    className={`w-full min-w-0 ${SIZES[size]} flex items-center gap-3 rounded-xl border-2 text-left transition-all duration-200 ${
                        isOpen
                            ? 'border-primary-500 ring-4 ring-primary-500/20'
                            : error
                              ? 'border-danger-500'
                              : 'border-slate-200 dark:border-slate-700'
                    } bg-white disabled:cursor-not-allowed disabled:opacity-60 dark:bg-slate-900`}
                >
                    {icon && (
                        <span
                ...
                {isOpen && (
                    <div
                        ...
                        className={`absolute left-0 right-0 top-full z-50 mt-2 w-full max-w-[calc(100vw-2rem)] animate-slide-up overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl dark:border-slate-700 dark:bg-slate-900 ${menuClassName}`}
                    >
                        {shouldSearch && (
            ...
            {error && (
                <small className="text-xs text-danger-500 dark:text-danger-400">{error}</small>
            )}
        </div>
    );
}
