...
export default function UpdateNotificationPreferences({ preferences }) {
    ...
    const enableAll = () =>
        setData(
            'preferences',
            Object.fromEntries(NOTIFICATION_TYPES.map((type) => [type.key, true]))
        );

    const enabledCount = NOTIFICATION_TYPES.filter((type) => data.preferences[type.key]).length;
            ...
            <form onSubmit={submit}>
                <header className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
                    <div>
                        ...
                        <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                            {enabledCount} dari {NOTIFICATION_TYPES.length} jenis notifikasi aktif
                            untuk akun Anda.
                        </p>
                    </div>
                ...
                <ul className="divide-y divide-slate-100 rounded-xl border border-slate-200 dark:divide-slate-800 dark:border-slate-800">
                    {NOTIFICATION_TYPES.map((type) => {
                        ...
                        return (
                            <li key={type.key} className="flex items-center gap-3 p-4">
                                <span
