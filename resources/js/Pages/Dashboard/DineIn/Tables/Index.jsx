...
function TableShape({ table, onDragStart, isSelected, onClick }) {
    ...
    return (
        <g {...dragProps} onClick={() => onClick && onClick(table)} className="cursor-pointer">
            {shape === 'circle' ? (
                <circle
            ...
            <text
                ...
                className={`${textClass} pointer-events-none select-none text-xs font-semibold`}
                fontSize={10}
            >
                {name}
            </text>
...
export default function Index({ tables, areas, filters }) {
    ...
    const submit = (e) => {
        ...
        const onSuccess = () => {
            toast.success(
                editingTable ? 'Meja berhasil diperbarui.' : 'Meja berhasil ditambahkan.'
            );
            setModalOpen(false);
        };
    ...
    const handleFilterArea = (areaId) => {
        ...
        router.get(route('dine-tables.index'), areaId ? { area_id: areaId } : {}, {
            preserveScroll: true,
        });
    };
            ...
            {activeView === 'grid' ? (
                <div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
                    <svg
                        ...
                        className="min-h-[500px] w-full rounded-xl bg-slate-50 dark:bg-slate-800"
                        onDrop={handleDrop}
                                ...
                                >
                                    <Table.Td>
                                        <div className="flex items-center gap-2">
                                            <span
                                                className={`inline-block h-3 w-3 rounded-sm ${table.shape === 'circle' ? 'rounded-full' : ''} ${table.is_active ? 'bg-primary-500' : 'bg-slate-300 dark:bg-slate-600'}`}
                                            />
                                        ...
                                        <div className="flex justify-end gap-2">
                                            {canAccess && (
                                                <Button
                                                    ...
                                                    className={
                                                        'border border-slate-200 bg-slate-100 text-slate-600 hover:bg-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-400'
                                                    }
                                                />
                                            )}
                                            {canUpdate && (
                ...
                <form onSubmit={submit} className="space-y-4">
                    ...
                    <div>
                        ...
                        <Select
                            ...
                            onChange={(value) => setData('dine_area_id', value)}
                            options={[
                        ...
                        />
                        {errors.dine_area_id && (
                            <p className="mt-1 text-xs text-danger-500">{errors.dine_area_id}</p>
                        )}
                    </div>
