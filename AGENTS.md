...
- `no-unused-vars` is a `warn` with `^_` escape hatches. Cleared to **1** in Sept 2026 (was 65): dead imports, dead `catch (e)` bindings, write-only `useState` in-flight trackers (`updatingCartId`, `removingItemId`, `pendingSyncCount` in `Transactions/Index`), and an entirely dead `refreshPendingCount` chain were removed. The one holdout is `handlePrintSingleBarcode` in `Products/Index` — real groundwork, no button ever wired to it.
- **Two `static-components` warnings were real bugs, not noise.** `NotificationList` was defined *inside* `Notification.jsx`'s render, so it was a fresh component type every pass and React remounted the list (losing scroll position) on each parent render. Fixed by calling it as a plain function — the same convention `Utils/Menu.jsx` already uses.
...
- **CI runs `npm run lint`, not `lint:strict`.** As of Sept 2026: **0 errors, 74 warnings** (from 141). Every remaining warning is a React Compiler-era rule across 27 files — 37 `set-state-in-effect`, 32 `exhaustive-deps`, 4 `immutability`, 1 `no-unused-vars`. These need real refactors, not config, and **this repo has no JS test runner**, so `npm run build` proves only that the code compiles — a behaviour refactor here is unverifiable by CI. Flip to `lint:strict` only once those are genuinely fixed and manually exercised.
...
- **Offline sync needs `$middleware->statefulApi()` on the api group.** The POS page is session-authenticated and posts to `/api/v1/pos/transactions/sync` with a plain cookie, but `auth:sanctum` only accepted a Bearer token — every queued sale got a 401 and sat in IndexedDB forever, silently. Sanctum only falls back to the session for hosts in `config('sanctum.stateful')`, so serving on a non-default port or host needs `SANCTUM_STATEFUL_DOMAINS`. `PosTransactionSyncTest` cannot catch a regression here (it uses `Sanctum::actingAs()`, which fakes a token); `PosOfflineSyncSessionTest` asserts the middleware is registered instead.
...
- **Dead keys in `.env.production.example`:** `MIDTRANS_CLIENT_KEY` and `MIDTRANS_IS_PRODUCTION` are never read. `config/services.php` only reads `MIDTRANS_SERVER_KEY`, `XENDIT_SECRET_KEY`, `XENDIT_CALLBACK_TOKEN`; client keys and sandbox/production mode live in the **`payment_settings` table** (with env fallbacks). `docs/configuration.md` no longer lists `XENDIT_PUBLIC_KEY` (nothing ever read it); `MIDTRANS_CLIENT_KEY` / `MIDTRANS_IS_PRODUCTION` are still physically present in `.env.production.example` — harmless, but don't trust them.
...
`docs/getting-started.md` · `docs/architecture-overview.md` · `docs/configuration.md` · `docs/feature-index.md` · `docs/testing-manual.md` (manual QA for QRIS, ESC/POS, offline sync) · `docs/features/` (28 module docs) · `docs/agents/{issue-tracker,triage-labels,domain}.md` (skills: local `.scratch/<slug>/` issue tracker, triage labels, domain docs) · `docs/superpowers/specs/`. Also `CONTRIBUTING.md` (branch + commit + gate rules), `CHANGELOG.md` (Keep-a-Changelog; tags are authoritative), `SECURITY.md` (v2.x only), `LICENSE` (MIT).

Some prose is stale — trust config over docs. The Sept 2026 doc-drift sweep **closed** all of these: `README.md` now documents the real default accounts (and that `UserSeeder` skips them in production), the correct `docs/features/` count, and a Docker section explaining the bundled Gotenberg; `docs/configuration.md` no longer lists `XENDIT_PUBLIC_KEY`; `.env.production.example` no longer claims a `gotenberg` compose service; `deploy.sh` no longer advertises the non-existent `ps` subcommand; and all 28 feature docs are now listed in `docs/feature-index.md`. Remaining gaps that are still true: `README.md` still says `PUPPETEER_SKIP_DOWNLOAD=true` in its Quick Start, which is a vestigial no-op now that `puppeteer` is gone. `CONTEXT.md` and `docs/adr/` are a documented convention (`docs/agents/domain.md`) with **no artifacts yet**; `.scratch/` exists but is empty.
